...
class StatAnalysis:
    ...
    def get_text_result(self, age, gender, inference_result):
        ...
        text = '{}살 ({}) HTP Test 결과를 출력합니다.'.format(age, gender)
        for k in self.ret:
            mu, percentile = self.ret[k]
            text += '{}: 평균 {}, 백분위수: {:.1f}'.format(k, mu, percentile)
    ...
    def _draw_dist(self, title, total_stat, sample_value, bins=30, color='skyblue'):
        # Create a histogram
        n, bins, patches = plt.hist(total_stat, bins=bins, edgecolor='black', color=color)    
